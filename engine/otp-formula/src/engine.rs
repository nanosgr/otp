...
fn failed(def: &ConceptoDef, msg: String) -> ConceptoRes {
    ConceptoRes {
        codigo: def.codigo.clone(), descripcion: def.descripcion.clone(), columna: def.columna, orden: def.orden,
        unidad: None, unitario: None, importe: Decimal::ZERO, decimales_importe: def.decimales_importe, condicion: true, error: Some(msg),
    }
}

fn eval_concept(def: &ConceptoDef, c: &Compiled, env: &mut Env) -> Result<ConceptoRes> {
    ...
    let condicion = match &c.condicion {
        Some(e) => env.eval(e)?.boolean()?,
        None => true,
    };
    let mut res = ConceptoRes {
        codigo: def.codigo.clone(), descripcion: def.descripcion.clone(), columna: def.columna, orden: def.orden,
        unidad: None, unitario: None, importe: Decimal::ZERO, decimales_importe: def.decimales_importe, condicion, error: None,
    };
    if !condicion {
        return Ok(res);
    }
    if let Some(e) = &c.unidad {
        let u = round_half_up(env.eval(e)?.num()?, def.decimales_unidad);
        env.scopes[0].insert("UNIDAD".to_string(), Value::Num(u));
        res.unidad = Some(u);
    }
    if let Some(e) = &c.unitario {
        let u = round_half_up(env.eval(e)?.num()?, 4);
        env.scopes[0].insert("UNITARIO".to_string(), Value::Num(u));
        res.unitario = Some(u);
    }
    let importe = match (&c.importe, res.unidad, res.unitario) {
        (Some(e), _, _) => env.eval(e)?.num()?,
        (None, Some(u), Some(p)) => u.checked_mul(p).ok_or_else(|| FormulaError::eval("Desbordamiento numérico"))?,
        _ => Decimal::ZERO,
    };
...
pub fn calcular(reglas: &Reglas, ctx: &Contexto) -> Result<Resultado> {
    ...
    let (order, remaining) = toposort(&defs, &deps);
    let cycle = find_cycle(&remaining, &deps);

    let defs_map: HashMap<String, ConceptoDef> = reglas.conceptos.iter().map(|d| (d.codigo.clone(), d.clone())).collect();
    ...
    for code in &order {
        ...
        let res = if !c.errors.is_empty() {
            failed(def, c.errors.iter().map(|(f, e)| format!("{f}: {e}")).collect::<Vec<_>>().join("; "))
        } else if let Some(m) = sel_errors.get(code) {
            failed(def, m.join("; "))
        } else {
            let mut env = Env { ctx, defs: &defs_map, grupos: &reglas.grupos, results: &results, current: code, scopes: Vec::new() };
            match eval_concept(def, c, &mut env) {
                Ok(r) => r,
                Err(e) => failed(def, e.to_string()),
            }
        };
        results.insert(code.clone(), res);
    }
    for code in &remaining {
        let msg = if cycle.contains(code) {
            format!("Dependencia circular: {}", cycle.iter().map(|c| format!("#{c}")).collect::<Vec<_>>().join(" -> "))
        } else {
            "Depende de conceptos con dependencia circular".to_string()
        };
        results.insert(code.clone(), failed(defs[code], msg));
    }
    ...
    Ok(Resultado { conceptos, orden_evaluacion: order, totales: t })
}
...
pub fn validar_reglas(reglas: &Reglas, variables_conocidas: Option<&BTreeSet<String>>) -> Result<Vec<Problema>> {
    ...
    let (deps, sel_errors) = build_deps(&defs, &compiled, &reglas.grupos);
    for (code, msgs) in sel_errors {
        for m in msgs {
            out.push(Problema { codigo: code.clone(), campo: String::new(), nivel: "error", mensaje: m });
        }
    }
    let (_, remaining) = toposort(&defs, &deps);
    if !remaining.is_empty() {
        let cycle = find_cycle(&remaining, &deps);
        for code in cycle.iter().collect::<BTreeSet<_>>() {
            out.push(Problema {
                codigo: code.clone(), campo: String::new(), nivel: "error",
                mensaje: format!("Dependencia circular: {}", cycle.iter().map(|c| format!("#{c}")).collect::<Vec<_>>().join(" -> ")),
            });
        }
    }
    Ok(out)
